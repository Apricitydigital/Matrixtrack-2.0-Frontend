...
export default function BeatStaffAssignmentsTab() {
  const [viewMode, setViewMode] = useState<"supervisor" | "employee">(
    "supervisor",
  );
  ...
  const visibleWards = selectedZone
    ? allWards.filter(
        (w) => w.parentId === selectedZone || w.parent?.id === selectedZone,
      )
    : allWards;
    ...
    return beats
      .filter((beat) => {
        const matchesSearch =
          ...
          (beat.supervisorsSummary || []).some((s: any) =>
            s.name?.toLowerCase().includes(q),
          );

        const matchesZone =
          !selectedZone ||
          beat.zoneId === selectedZone ||
          beat.zoneName === zones.find((z) => z.id === selectedZone)?.name;

        const selectedWardObj = allWards.find((w) => w.id === selectedWard);

        const matchesWard =
          !selectedWard ||
          beat.wardId === selectedWard ||
          beat.wardName === selectedWardObj?.name;
  ...
  const exportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `beat_staff_assignments_${new Date().toISOString().slice(0, 10)}.csv`,
    );
  ...
  const totalSupervisors = useMemo(() => {
    ...
    beats.forEach((beat: any) => {
      (beat.supervisorsSummary || []).forEach((supervisor: any) => {
        const key = String(
          supervisor?.id || supervisor?.userId || supervisor?.name || "",
        ).trim();
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            📍 Total Beat Assignments: <strong>{filteredBeats.length}</strong>{" "}
            Beats
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        >
          ...
          >
            No beat assignments match your filters
          </h4>
          <p style={{ fontSize: 12, margin: "4px 0 0" }}>
            Try clearing filters or search criteria above.
          </p>
        </div>
      ) : (
        <div
          ...
          >
            ...
            <tbody>
              {filteredBeats.map((beat) => {
                ...
                const assignedSegs = (beat.segments || []).filter(
                  (s: any) => !!s.employeeAssignedToId,
                ).length;
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {beat.wardName || "Ward 1"}
                      </div>
                      <div style={{ fontSize: 11, color: "#64748b" }}>
                        {beat.zoneName || "Zone 1"}
                      </div>
                    </td>
                      ...
                      {isSupervisorView ? (
                        ...
                        (() => {
                          ...
                          (beat.segments || []).forEach((s: any) => {
                            const eName =
                              s.employeeAssignedTo?.name ||
                              s.employeeAssignedName ||
                              s.employeeName ||
                              s.assignedEmployee?.name ||
                              s.employee?.name;
                            if (eName)
                              empMap.set(eName, (empMap.get(eName) || 0) + 1);
                          });
                          ...
                          if (
                            ...
                            beat.employeesSummary.forEach((e: any) => {
                              ...
                              if (eName && !empMap.has(eName))
                                empMap.set(eName, e.segmentCount || 1);
                            });
                          }

                          const empList = Array.from(empMap.entries()).map(
                          ...
                          return empList.length > 0 ? (
                            ...
                            <div
                              style={{
                                ...
                                fontWeight: 600,
                                color:
                                  assignedSegs === totalSegs && totalSegs > 0
                                    ? "#15803d"
                                    : "#334155",
                              }}
                            >
