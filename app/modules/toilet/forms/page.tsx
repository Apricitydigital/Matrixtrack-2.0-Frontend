...
export default function ToiletFormsPage() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      // IEC Forms for Toilet not yet implemented; placeholder load
      setForms([]);
      setError("");
    } catch (err) {
  ...
  const createForm = async (e: React.FormEvent) => {
    ...
    try {
      // IEC forms for Toilet are not yet implemented in the backend.
      setCreateStatus("IEC Forms feature coming soon.");
    } catch (err) {
    ...
    <ModuleGuard module="TOILET" roles={["EMPLOYEE", "QC", "ACTION_OFFICER", "CITY_ADMIN", "HMS_SUPER_ADMIN"]}>
      <div className="grid grid-2">
        ...
        <div className="card">
          ...
          {!loading && !forms.length && !error && <p>No forms available. This feature is coming soon.</p>}
