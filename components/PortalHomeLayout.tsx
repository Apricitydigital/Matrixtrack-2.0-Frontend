...
function PortalHomeLayoutContent({
  ...
  const isSwachhActive =
    pathname.startsWith('/ward-ranking') && !pathname.startsWith('/ward-ranking-system');

  const isWorkforceActive =
    pathname.startsWith('/workforce-monitoring');

  const isWardRankingSystemActive =
    pathname.startsWith('/ward-ranking-system');
  ...
  const [
    wardRankingSystemOpen,
    setWardRankingSystemOpen,
  ] = useState(isWardRankingSystemActive);
  ...
  useEffect(() => {
    ...
    if (isWardRankingSystemActive) {
      setWardRankingSystemOpen(true);
    }
  }, []);
  ...
  if (isTaskforceActive) {
    ...
    if (
      ...
      if (pathname.includes('/ward-ranking-system')) {
        pageTitle = 'Ward & Zone Performance Dashboard';
      } else {
        pageTitle =
  ...
  if (isWardRankingSystemActive) {
    pageTitle = pathname.startsWith('/ward-ranking-system/employee-attendance')
      ? 'Employee Attendance Analytics'
      : 'Ward & Zone Performance Dashboard';
  }
    ...
    <div className="min-h-screen bg-slate-50/50 flex">
            ...
            {isCityAdmin && (
              <div className="flex flex-col mb-1">
                ...
                >
                  <ChartNoAxesCombined
                    size={18}
                    className={pathname === '/city/attendance' ? 'text-blue-600' : 'text-slate-500'}
                  />
                  <span className="text-left leading-snug">Attendance Analytics</span>
                </Link>
              </div>
            )}
              ...
              <div className="flex flex-col mb-1">
                ...
                >
                  <div className="flex items-center gap-3 text-left min-w-0">
                    <Award
                      size={18}
                      className={isWardRankingSystemActive ? 'text-blue-600' : 'text-slate-500'}
                    />
                  ...
                  <ChevronDown
                    ...
                    className={`shrink-0 transition-transform duration-200 ${
                      wardRankingSystemOpen ? 'rotate-180 text-blue-600' : 'text-slate-400'
                    }`}
                  />
                </button>
                ...
                {wardRankingSystemOpen && (
                  <div className="ml-4 mt-1 pl-3 border-l-2 border-blue-200 flex flex-col gap-1.5">
                    ...
                    >
                      <LayoutDashboard
                        size={15}
                        className={pathname === '/ward-ranking-system' ? 'text-white' : 'text-slate-400'}
                      />
                      <span>Dashboard</span>
                    </Link>

                    <Link
                      href="/ward-ranking-system/employee-attendance"
                      className={`flex items-center gap-2.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 ${
                        pathname.startsWith('/ward-ranking-system/employee-attendance')
                          ? 'bg-blue-600 text-white font-bold shadow-sm shadow-blue-500/20'
                          : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
                      }`}
                    >
                      <Users
                        size={15}
                        className={pathname.startsWith('/ward-ranking-system/employee-attendance') ? 'text-white' : 'text-slate-400'}
                      />
      ...
      <main className="portal-main-content flex-1 ml-72 p-5 sm:p-6 min-w-0 min-h-screen text-slate-800">
        ...
        {!isMainDashboard && (
          ...
          <div
            className={`mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-white rounded-3xl border border-slate-800/80 shadow-xl transition-all overflow-hidden relative ${
              isWardRankingSystemActive
                ? 'p-8 md:py-10 md:px-10 relative mb-6'
                : 'px-6 py-3.5 sticky top-3 z-40'
            }`}
            style={{
              background:
          ...
          >
            ...
            <div className="z-10">
              ...
              <div className="text-[10px] font-black uppercase tracking-[0.18em] text-blue-400 flex items-center gap-1.5 mb-1.5">
                ...
                {cityName && (
                  <>
                    ...
                    <span className="bg-blue-500/20 text-blue-300 px-2 py-0.5 rounded-full text-[9px] font-bold tracking-normal uppercase">
                      {cityName}
                    </span>
                  </>
                )}
              ...
              <h1 className={`font-black text-white tracking-tight leading-tight ${
                isWardRankingSystemActive ? 'text-2xl sm:text-3xl' : 'text-lg sm:text-xl'
              }`}>
                {pageTitle}
              </h1>

              {isWardRankingSystemActive && (
                <p className="text-xs text-slate-300 mt-2 max-w-xl font-medium">
                  {pathname.startsWith('/ward-ranking-system/employee-attendance')
                    ? 'Ward-wise employee attendance, roster status, punch-in timelines, and mapped workforce visibility across the city.'
                    : 'Real-time municipal performance analytics and target completion scores mapped across all zones and wards.'}
                </p>
              )}
